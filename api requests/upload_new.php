...
if (isset($_REQUEST['err']) && ($_REQUEST['err']=="invalidFileType"))
{
	...
	echo 'ERROR: Invalid File Type, Must be a PDF File!</div>';
}
elseif (isset($_REQUEST['err']) && ($_REQUEST['err']=="invalidLoan"))
{
	echo '<div class="alert alert-danger alert-dismissable">';
	echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
	echo 'ERROR: Invalid Loan ID, Must be 8 Digits!</div>';
}
	...
	else
	{
		...
		if (pathinfo($_FILES['userFile']['name'], PATHINFO_EXTENSION) != 'pdf')
		{
			...
			if ($file)
			{
				fwrite($file, $msg);
				fclose($file);
				redirect("upload_new.php?err=invalidFileType");
			}
		}
		else
		{
			...
			$sql="Select `auto_id` from `files` where `loanID`='$loanNum'";
			$result=$dblink->query($sql) or
