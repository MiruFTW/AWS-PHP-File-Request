...
if (isset($_REQUEST['err']) && ($_REQUEST['err']=="InvalidType"))
{
	echo '<div class="alert alert-danger alert-dismissable">';
	echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
...
elseif (isset($_POST['submit']) && $_POST['submit']=="submit")
{
	...
	echo '<h2>Results from file type: '.$fileType.'</h2>';
	$sql="SELECT `auto_id`,`file_name`,`loanID`,`file_type`,`file_status`,`file_size`,`file_datetime` FROM `files` WHERE `file_type`='$fileType'";
	$result=$dblink->query($sql);
	if (!$result) // query error
	{
		$file = fopen($errorPath, 'a');
	...
	echo '<thead><tr><td>File Name</td><td>Loan ID</td><td>Upload Method</td><td>File Size</td>
	<td>File Upload Date Time</td><td>Action</td></tr></thead>';
