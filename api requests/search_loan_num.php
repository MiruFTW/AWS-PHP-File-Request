...
if (isset($_REQUEST['err']) && ($_REQUEST['err']=="NotFound"))
{
	...
	echo 'Loan Number was NOT found in the system!</div>';
}
if (isset($_REQUEST['err']) && ($_REQUEST['err']=="InvalidLoan"))
{
	echo '<div class="alert alert-danger alert-dismissable">';
	echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
	echo 'Invalid Loan Number Entered! Must Be 8 Digits!</div>';
}
...
elseif (isset($_POST['submit']) && $_POST['submit']=="submit")
{
	...
	if (!preg_match('/^\d{8}$/',$loanNum))
	{
		redirect("search_loan_num.php?err=InvalidLoan");
	}
	$dblink=db_connect("main");
	$sql="SELECT DISTINCT `loanID` FROM `files` WHERE `loanID`='$loanNum'";
	...
	{
		redirect("https://ec2-18-222-24-134.us-east-2.compute.amazonaws.com/search_loan_num.php?err=NotFound");
	}
	else // loan num was found: display results
	{
		echo '<h2>Results from loan number: '.$loanNum.'</h2>';
		echo '<table id="myTable" class="display" style="width:100%">';
		echo '<thead><tr><td>File Name</td><td>File Type</td><td>Upload Method</td><td>File Size</td><td>File Upload Date Time</td><td>Action</td></tr></thead>';
		//$loan=$result->fetch_array(MYSQLI_ASSOC);
		$sql="SELECT `auto_id`,`file_name`,`loanID`,`file_type`,`file_status`,`file_size`,`file_datetime` FROM `files` WHERE `loanID`='$loanNum'";
		$result=$dblink->query($sql);
		if (!$result) // query error
		{
			$file = fopen($errorPath, 'a');
